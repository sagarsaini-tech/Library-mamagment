      ...
      <div id="printable-report" className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-slate-800">₹{(stats?.totalIncome || 0).toLocaleString()}</h3>
          </div>
          <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm">
            <div className="flex justify-between items-start mb-2">
            ...
            <h3 className="text-2xl font-bold text-slate-800">₹{(stats?.totalExpense || 0).toLocaleString()}</h3>
          </div>
          <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm">
            <div className="flex justify-between items-start mb-2">
          ...
          <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm">
            ...
            <h3 className="text-2xl font-bold text-slate-800">₹{(stats?.pendingFeesAmount || 0).toLocaleString()}</h3>
          </div>
        </div>
              ...
              {expenseCategoryData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <Tooltip formatter={(value: number) => [`₹${value?.toLocaleString?.() || value}`, 'Amount']} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                  </PieChart>
                </ResponsiveContainer>
              ) : (
