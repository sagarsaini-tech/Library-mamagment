...
import { LayoutDashboard, User, CalendarCheck, Receipt, Bell, Settings, LogOut, Library, Menu, X, ChevronDown } from 'lucide-react';
...
export const StudentLayout: React.FC = () => {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/student/dashboard', icon: <LayoutDashboard className="w-5 h-5 mr-3" /> },
    { name: 'My Profile', path: '/student/profile', icon: <User className="w-5 h-5 mr-3" /> },
    { name: 'Attendance', path: '/student/attendance', icon: <CalendarCheck className="w-5 h-5 mr-3" /> },
    { name: 'Fee Details', path: '/student/fees', icon: <Receipt className="w-5 h-5 mr-3" /> },
    { name: 'Notifications', path: '/student/notifications', icon: <Bell className="w-5 h-5 mr-3" /> },
    { name: 'Settings', path: '/student/settings', icon: <Settings className="w-5 h-5 mr-3" /> },
  ];
    ...
    <div className="flex h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden">
      ...
      <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:flex md:flex-col ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 border-b border-slate-200 flex items-center justify-between">
          ...
          <button className="md:hidden text-slate-500 hover:text-slate-700 focus:outline-none" onClick={() => setSidebarOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
