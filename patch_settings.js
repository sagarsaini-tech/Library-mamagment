...
export const Settings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        const settingsDoc = await getDoc(doc(db, \`libraries/\${user.uid}/settings/config\`));
        if (settingsDoc.exists()) {
          ...
          const libDoc = await getDoc(doc(db, 'libraries', user.uid));
          if(libDoc.exists()) {
  ...
  const uploadImage = async (file: File, type: 'logo' | 'cover'): Promise<string> => {
    if (!user) return '';
    try {
      const storageRef = ref(storage, \`libraries/\${user.uid}/\${type}_\${Date.now()}\`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, \`libraries/\${user.uid}/settings/config\`), configData, { merge: true });
      
      // Also update basic info in main library doc for backward compatibility
      await setDoc(doc(db, 'libraries', user.uid), {
...
fs.writeFileSync('src/pages/owner/Settings.tsx', content);
