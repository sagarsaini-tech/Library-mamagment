...
const ROLES = ['MANAGER', 'RECEPTIONIST', 'ACCOUNTANT', 'LIBRARIAN', 'STAFF'];
const PERMISSIONS = [
...
export const StaffManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    role: 'RECEPTIONIST', status: 'Active', joiningDate: ''
  });
  ...
  useEffect(() => {
    ...
    const staffRef = collection(db, \\\`libraries/\\\${user.uid}/staff\\\`);
  ...
  const handleAddStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, \\\`libraries/\\\${user.uid}/staff\\\`, staffUid), {
        staffId: staffUid,
        employeeId: empId,
        fullName: formData.fullName,
  ...
  const handleTogglePermission = async (staffId: string, permission: string, currentPermissions: string[]) => {
    ...
    try {
      ...
      await updateDoc(doc(db, \\\`libraries/\\\${user.uid}/staff\\\`, staffId), {
        permissions: updated,
        updatedAt: new Date().toISOString()
      });
    } catch (error) {
  ...
  const handleDeleteStaff = async (staffId: string) => {
    ...
    if(window.confirm('Are you sure you want to remove this staff member? This will not delete their Auth account completely, but removes access.')) {
      try {
        await deleteDoc(doc(db, \\\`libraries/\\\${user.uid}/staff\\\`, staffId));
