...
export const ExpenseManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingExpense) {
        const docRef = doc(db, \`libraries/\${user.uid}/expenses\`, editingExpense.id);
        await setDoc(docRef, expenseData, { merge: true });
        showToast('success', 'Expense updated successfully');
      } else {
        const newRef = doc(collection(db, \`libraries/\${user.uid}/expenses\`));
  ...
  const handleDelete = async () => {
    ...
    setFormSaving(true);
    try {
      await deleteDoc(doc(db, \`libraries/\${user.uid}/expenses\`, deletingId));
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 no-print">
        <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm flex flex-col justify-between">
          ...
          <h3 className="text-2xl font-bold text-slate-800">₹{stats.totalMonthlyIncome.toLocaleString()}</h3>
        ...
        <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm flex flex-col justify-between">
          ...
          <h3 className="text-2xl font-bold text-slate-800">₹{stats.totalMonthlyExpenses.toLocaleString()}</h3>
          <p className="text-xs text-slate-400 mt-1">Current Month</p>
        </div>
        ...
        <div className="bg-white rounded-xl p-5 border border-slate-200/60 shadow-sm flex flex-col justify-between">
          ...
          <h3 className="text-2xl font-bold text-slate-800">₹{stats.pendingExpensesAmount.toLocaleString()}</h3>
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedExpenses.map(expense => (
                  <tr key={expense.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm font-bold text-slate-800">₹{expense.amount.toLocaleString()}</p>
                    </td>
                    <td className="px-6 py-4">
...
console.log('ExpenseManagement.tsx written successfully');
